...
const RoomTypesManager = ({
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (typeof fetchRoomTypes === 'function') {
        fetchRoomTypes(searchQuery);
      }
    }, 300);

    return () => clearTimeout(timer);
  }, [searchQuery, fetchRoomTypes]);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">
                {editingRoom ? `Cấu hình thông tin cho: ${editingRoom.name}` : 'Nhập thông tin danh mục phòng ngủ và đơn giá'}
              </p>
            </div>
          </div>
        </div>
