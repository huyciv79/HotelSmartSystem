  ...
  return (
    <div className="space-y-6 animate-scale-in text-left">
      <div className="flex justify-between items-end border-b border-neutral-900 pb-4">
        <div>
          <h3 className="text-white font-black text-base uppercase tracking-wider m-0">QUẢN LÝ DỊCH VỤ</h3>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                className="w-full bg-transparent border border-neutral-800 p-3 font-bold text-xs outline-none text-white focus:border-primary resize-none placeholder:text-slate-650"
              />
            </div>
            ...
            <tbody className="divide-y divide-neutral-900/60">
              {services.map((svc) => (
                <tr key={svc.id} className="hover:bg-white/5 transition-colors">
                  <td className="p-4 pl-6 text-slate-500 font-bold">
                    #{svc.id}
                  </td>
                  <td className="p-4">
                    <span className="text-white font-black uppercase text-sm block">{svc.name}</span>
                  </td>
                  <td className="p-4 max-w-xs truncate font-medium text-slate-400">
                    {svc.description || <span className="text-slate-600 italic">Không có mô tả</span>}
                  </td>
